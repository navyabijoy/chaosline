    ...
    async (input) => {
      try {
        const result = await Promise.race([
          ...
          new Promise(
            (_, reject) => setTimeout(() => reject(new Error(`tool call ${mcpTool.name} timed out after ${PER_CALL_TIMEOUT_MS}ms`)), PER_CALL_TIMEOUT_MS)
          )
        ]);
...
--- turn ${turn} ---`);
        ...
        const textContent = response.content.filter((c) => typeof c === "string").join("\n");
        ...
        for (const toolUse of toolUses) {
          ...
          const result = await toolFn.invoke(toolUse.args);
