    ...
    (async () => {
      ...
      while (turn < MAX_TURNS) {
        ...
        const response = await claudeAgent.messages.create({
          ...
          messages: messages as Parameters<typeof claudeAgent.messages.create>[0]["messages"],
        });
        ...
        const toolUses = response.content.filter((b: unknown) => typeof b === "object" && b !== null && (b as Record<string, unknown>).type === "tool_use");
        const textBlocks = response.content.filter((b: unknown) => typeof b === "object" && b !== null && (b as Record<string, unknown>).type === "text");

        if (textBlocks.length) {
          finalText = textBlocks.map((b: unknown) => (typeof b === "object" && b !== null ? (b as Record<string, unknown>).text : "")).join("\n");
        }
